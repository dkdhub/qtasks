...
(defn make-job
  ...
  [{:keys [ns name arguments identity group desc recovery durably state]
    ...
    :as   opts}]
  (-> (JobBuilder/newJob (if (contains? opts :state) QtasksStatefullJob QtasksJob))
...
(defn schedule-job
  ...
  (let [job (job-from-var var (assoc job :arguments arguments))
    ...
    (.scheduleJob (:qtasks/quartz scheduler) job #{trigger} replace)
    job))

(defn delete-job
  [scheduler & key]
  (.deleteJob (:qtasks/quartz scheduler) (apply job-key key)))

(defn check-job-exists
  [scheduler & key]
  (.checkExists (:qtasks/quartz scheduler) (apply job-key key)))

(defn check-trigger-exists
  [scheduler & key]
  (.checkExists (:qtasks/quartz scheduler) (apply trigger-key key)))
